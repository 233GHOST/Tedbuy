...
- `users`: re-inventoried at the start of Phase 1 (16 direct write call sites confirmed via fresh grep, close to the original "~15" estimate) and broken into sub-checkpoints by feature, smallest/lowest-risk first:
  ...
  - ⬜ **Presence/analytics fields** (`fcmTokens`, `visitCount`, `lastLogin`, `lastSeen`, `isOnline`, `rapidPostScore`) — 6 call sites. **New finding, discovered during the Phase 1 inventory**: none of these fields are in `dbAdapter.ts`'s `TABLE_COLUMNS.users` allow-list, meaning these direct writes have likely been silently no-op'ing at the database level for some time already (the optimistic local React state still updates, so the UI wouldn't show anything wrong) — this looks like a pre-existing *functional* bug, unrelated to this security migration, not something introduced by any checkpoint so far. Needs its own investigation (confirm the no-op via a live test, decide whether to actually fix the underlying feature or just formalize the current de-facto-disabled state) before deciding how to migrate it — not a drop-in.
  - ⬜ **`updateUserProfile`** (the main profile-save action — username/bio/phone/whatsApp/photoUrl/role) — 1 call site (already calls `syncUserToServer` in parallel with its direct write, same shape as `savedProductIds` before checkpoint 5; likely a similarly small, safe checkpoint once reached).
  - ⬜ **Registration / Google sign-up / account-migration-merge flows** — remaining call sites, including the account-migration `writeBatch` that both creates a merged document and deletes a stale one (two different `users` ids in one operation) and the registration flow's atomic `users` + `store_names` batch. Highest stakes in this whole table (breaking registration or login is catastrophic) — deliberately last, and likely needs its own multi-step design pass rather than a single checkpoint, the same way the products views/likes residual did in Phase 0.
- `messages`: finish the `mark-read` migration; build the new `delete` endpoint. Not started.
- `store_names`: extend write coverage to the 3 flows not yet covered. Not started.
...
**Current state: Phase 0 is complete and Phase 1 has begun**, both being worked as a sequence of small, isolated, individually-reviewed checkpoints rather than large batches. Phase 0: `94c3cc6` (item 1, users bulk PII leak), `33441f1` (item 2, products views/likes residual), `409d2f4` (item 3, notificationPreferences cross-user write), `aeba056` (item 4, three-table unmapping) — items 1 and 2 pending the reviewer's own live/manual verification, items 3 and 4 fully verified. One adjacent finding from checkpoint 3 (`updateProduct()`'s social-only `dbAdapter` write branch) remains documented but not fixed, its own future item. Phase 1: `7702c75` (checkpoint 5, the `savedProductIds` slice of the `users`-table write migration) — fully verified, no server-side change involved. A new functional (not security) finding surfaced while inventorying the rest of Phase 1's `users`-table work: several presence/analytics fields (`fcmTokens`, `visitCount`, `lastLogin`, `lastSeen`, `isOnline`, `rapidPostScore`) appear to have been silently no-op'ing at the database level for some time, unrelated to any checkpoint in this migration — flagged in §4's Phase 1 breakdown, not yet investigated further. No RLS, policy, grant, or schema change has been made at any point. Phases 2-5 in full, and most of Phase 1 (registration, login, profile-save, presence-tracking, `messages`, `store_names`), remain not yet started.
