...
| Edit — direct-Supabase parallel path (views/likes only) | `AppContext.tsx` `toggleLikeProduct`, `incrementProductViews` — `updateDoc(doc('products', id), {...})` | `updateDoc` | `TABLE_COLUMNS` field allow-list only, no ownership check; `increment()` helper computes "current + delta" **client-side**, no server atomicity at all | MEDIUM (ranking-relevant — see §22 of the audit doc) | C (ranking signal) | `/api/products/sync`'s social-only path was fixed server-side this session (self-toggle + derived count) but this direct path still exists in parallel | Yes — needs an endpoint supporting **both anonymous** (view tracking) and **authenticated self-toggle** (likes), a real design decision, not a drop-in | Both features break entirely (can't like or record a view) | P1 |
...
1. A **views/likes endpoint** supporting both anonymous view-increment and authenticated self-toggle-like for `products` (§1.2).
