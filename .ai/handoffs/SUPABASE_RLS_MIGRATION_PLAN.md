...
**STATUS: BLOCKED_APPROVAL — RLS itself (Phase 4) still requires explicit approval.** All four original Phase 0 items are implemented, each as its own isolated checkpoint (`94c3cc6`, `33441f1`, `409d2f4`, `aeba056`) — items 1 and 2 pending live/manual verification, items 3 and 4 fully verified. One adjacent finding surfaced at checkpoint 3 remains open as a documented future item. **Phase 1's `users`-table write migration is essentially complete**: checkpoints 5-9 (`7702c75`, `1cf8e3e`, `ab4afba`, `95cd5e6`, `87f647f`), all fully verified, `store_names` write coverage resolved as a side effect. **Phase 2 has begun with two urgent, newly-discovered findings**: checkpoints 10-11 (`7877351`, `a7eff79`) closed a *second* bulk-PII-read vulnerability in the `users` table that the original Phase 0 sweep's own grep missed entirely — same severity as the original §0 finding, both fully verified. **One genuinely high-risk item remains deliberately deferred**: `findAndMigrateExistingUser`'s account-migration merge (creates a document under a new id and deletes one under an old id in the same operation) — needs real server-side verification design, not a rushed migration. Several other adjacent findings are also deliberately deferred, each documented at its own checkpoint (see §4 for the full list). Phases 3 through 5, and most of Phase 2, remain design-only or not yet started. RLS, policies, grants, schema, and production config remain completely untouched throughout.
...
- `users` self-profile realtime → replace with a poll of `/api/users/get?id=<own uid>` (matches the notifications migration's own precedent of trading realtime push for a poll — §18.5 of the audit doc — or, if instant reactivity to admin-side changes like suspension truly matters, a dedicated lightweight polling endpoint). Not started — deliberately not attempted alongside the account-migration-merge write (Phase 1) given both touch the same large, sensitive `onAuthStateChanged` handler; tackling both back-to-back risked compounding complexity.
- `chats`/`messages` admin support-desk realtime → the new admin-only polling endpoint from §3. Not started — needs genuinely new server infrastructure.
- `store_names` account-migration-detection read (step 5 of `findAndMigrateExistingUser`, a single-doc `getDoc` by an already-discovered userId) — **not migrated**, deliberately: it's a narrower, single-document lookup (not a bulk/query leak like the ones just fixed), and it's tightly coupled to the still-deferred account-migration-merge write logic (Phase 1, checkpoint 9's note) rather than a standalone finding worth separating out.
...
**Phase 2**: `7877351` (checkpoint 10) and `a7eff79` (checkpoint 11) closed a second, previously-undiscovered bulk/query-based PII-read vulnerability in `findAndMigrateExistingUser` — the original Phase 0 §0 finding was real but incomplete; this document's own confidence that it had found "the" bulk-read issue was wrong, a fact worth remembering when judging how exhaustively any grep-based sweep should be trusted. Both fully verified, no server-side change (reused existing endpoints).

**Deliberately deferred findings**, each documented at its own checkpoint rather than rushed or silently dropped: `updateProduct()`'s social-only `dbAdapter` branch (Phase 0 checkpoint 3); the store-name-index/`sellerName`-sync blocks inside `updateUserProfile` (checkpoint 7); `setupWelcomePackage`'s multi-table direct writes (checkpoint 8); `sendWelcomeEmailToAll`'s per-target `welcomeSent` write, which would need a new PATCH-style admin endpoint (checkpoint 9); `findAndMigrateExistingUser`'s single-doc `store_names`-candidate lookup (checkpoint 11, narrower than the fixed bulk/query reads, coupled to the still-deferred merge write); and the genuinely highest-risk item in the whole document, `findAndMigrateExistingUser`'s account-migration merge itself (creates a document under a new id and deletes one under an old id in one operation — needs real server-side verification design before it can be safely migrated, not a quick fix).

No RLS, policy, grant, or schema change has been made at any point. Phases 3-5 remain not yet started. Within Phase 2: `users` self-profile realtime, `messages`/`chats` admin support-desk realtime, and the `reviews` bulk-read hygiene migration are not yet started. The account-migration merge remains the single highest-value remaining item across the whole document.
