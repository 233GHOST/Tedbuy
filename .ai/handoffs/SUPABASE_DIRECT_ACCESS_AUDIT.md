...
The **only** ownership-adjacent check anywhere in this file is a data-integrity guard (not an authorization guard): `setDoc`/`updateDoc` refuse to write an ID that looks like a `chat_`/`prod_`/`msg_`/`notif_`/`report_`/`rev_`-prefixed value into the `users` table (lines ~889-901, ~961-964). Nothing anywhere checks "does the currently-signed-in Firebase user actually own this row." That check exists **only** as Postgres RLS — which is confirmed off. So today, the sole thing preventing `updateDoc(doc('products', 'someone_elses_product_id'), {...})` — or a raw PostgREST request bypassing the app's UI entirely — from succeeding for **any** row in **any** mapped table is: nothing. `VALID_TABLE_MAP` covers `users`, `products`, `chats`, `messages`, `notifications`, `reviews`, `reports`, `store_names`, `boost_purchases`, `admin_audit_logs`, `account_deletion_audits`.

- **[Corrected in §21.2 — this bullet's original claim was wrong, left here struck through rather than silently deleted]** ~~`reports` is in `VALID_TABLE_MAP` but not in `TABLE_COLUMNS` → `getDocPathInfo` nulls out the table for it → every `reports` operation via `dbAdapter` silently no-ops. Effectively unreachable through this path today.~~ Verified directly this pass: `mapPathToTable` (what `getDocPathInfo` actually calls) only ever consults `VALID_TABLE_MAP`, never `TABLE_COLUMNS` — a table missing from `TABLE_COLUMNS` does **not** get nulled out; `filterTableColumns` instead treats a missing entry as "don't filter" and lets every field through unfiltered. `reports` sat fully open, unfiltered, for the entire span this document covers. Confirmed real and fixed in §21.2.
...
## 19. Final repository-wide client-trust / business-logic sweep (IMPLEMENTED)

Requested explicitly as a closing pass before the RLS go/no-go decision: search the entire repo (direct Supabase writes AND authenticated server endpoints) for any client-controlled value able to influence money/payment, ownership, identity, admin/moderator privilege, suspension/deletion, moderation/visibility, trade completion, reviews/ratings, ranking/boosting, notifications, seller trust, privileged API behavior, or destructive operations — tracing each to its actual effect rather than trusting field names, and continuing past the point where "the obvious fields are clean."

### 19.1 `emailVerified` — client-trust in `/api/users/sync` (CONFIRMED VULNERABLE, FIXED)

`/api/users/sync` took `emailVerified` straight from the client body (`user.emailVerified === true`). Unlike `isAdmin`/`isSuspended` (§13/§14.1), simply preserving the existing DB value would have been the *wrong* fix here — a real, legitimate transition to `true` happens whenever a user actually clicks their Firebase verification link, and the client is expected to sync that. Fixed by deriving the value directly from Firebase Admin SDK's own record (`getAdminAuth().getUser(uid).emailVerified`) instead of trusting either the client's claim or a possibly-stale DB copy — independent of anything the request body says, with no functional impact on the legitimate "I just verified" case.

`dbAdapter.ts`'s `users` `TABLE_COLUMNS` still includes `emailVerified` for the direct-Supabase route (~12 read/write call sites across `AppContext.tsx`, mostly self-referential local-state mirroring of a real Firebase-verified fact). Left open this pass: the server-side fix closes the more consequential, more easily reachable HTTP-endpoint vector; the residual direct-Supabase exposure has a low ceiling (a user can only ever falsely mark *their own* email as verified locally/via raw Supabase, which the real verification gate — Firebase Auth itself — doesn't depend on) and is documented rather than migrated, consistent with the RLS-dependent residual-risk bucket discussed throughout this document.

### 20. Username quarantine — enforcement gap in `/api/users/sync` (CONFIRMED VULNERABLE, FIXED)

The 90-day username quarantine (`store_names.status = 'quarantined'`, applied on account deletion — see the deletion flow's "Quarantine Username / Store Name" step) was **written** on deletion but never actually **checked** at the real username-claim path. `GET /api/auth/check-store-name/:username` — the only place that ever read the quarantine — is a pure informational/UI-hint endpoint for a signup form's live-availability check; nothing stopped a user from ignoring it (or simply never calling it) and claiming a just-deleted user's exact username immediately via a normal profile save (`/api/users/sync`), silently overwriting the quarantine record itself since that endpoint's own `store_names` upsert is keyed by the same normalized-username id. This is an identity/impersonation issue (capturing a departed seller's exact username and accumulated reputation) that existed independently of RLS — it was simply never enforced anywhere.

**Fixed:** added a real server-side check in `/api/users/sync`, before any username claim is accepted — queries `store_names` for an active (`status === 'quarantined'`, `availableAfter` still in the future), differently-owned reservation and rejects the claim if found. Non-admin only; admin override preserved for legitimate support cases (e.g., a support-cleared reissue).

**Direct-Supabase route also closed:** `dbAdapter.ts`'s `store_names` `TABLE_COLUMNS` previously included `status`/`availableAfter`/`quarantinedAt` — the exact fields the quarantine system depends on — with no legitimate client write path needing them (the reservation flow only ever needs `id`/`userId`/`username`; confirmed via grep across all 4 client call sites in `AppContext.tsx`, all of which only ever set those three fields). Stripped to `id`/`userId`/`username` only.

### 21. `reviews`/`reports` direct-write gap, and ranking manipulation via `/api/products/sync` (CONFIRMED VULNERABLE, FIXED)

Three related findings, all touching the "trade completion / reviews-ratings / ranking-boosting" categories explicitly called out in the sweep request:

**21.1 (P1) `/api/products/sync` trusted `viewsCount`/`likesCount`/`likedUserIds` as absolute client-supplied values, used directly in ranking.** This endpoint has a "social-only bypass": if the *only* fields in a product-sync request differ from what's already saved are `likedUserIds`/`likesCount`/`viewsCount`/`id`, the normal seller-ownership check is skipped entirely — a deliberate, necessary design, since liking or viewing *someone else's* listing must not require owning it. The bug was in what happened next: `viewsCount`/`likesCount` were taken directly from the client body with no validation of the value itself (any caller could set either to any number for *any* listing, not just their own), and `likedUserIds` was trusted as a full array wholesale (any caller could inject or remove *other* users' ids, fabricating or erasing who liked a listing). Both `viewsCount` and `likesCount` feed directly into `engagementScore`/`priorityScore`, used in boost-tier tiebreaking and admin free-boost grants — so this was a real, unauthenticated-ownership-bypass-adjacent ranking-manipulation vector, reachable by any authenticated user with a normal HTTP client, no special tooling required.

Traced the app's actual like/view features (`toggleLikeProduct`, `incrementProductViews` in `AppContext.tsx`) and confirmed neither one actually calls this endpoint — both write directly via `dbAdapter` instead (see 21.3). No legitimate caller of `/api/products/sync` needs an absolute value trusted here at all.

**Fixed** in `upsertProductToSupabase`: `likedUserIds` now only ever toggles the *calling* user's own id relative to what's already saved (derived from `actingUser.uid`, never from the array's contents); `likesCount` is always derived from that array's resulting length, never taken independently; `viewsCount` is preserved from the existing row unconditionally (no feature legitimately claims an absolute view count through this endpoint, and there's no way to validate one anyway).

**21.2 (P1) `reports` had no `TABLE_COLUMNS` entry in `dbAdapter.ts` at all.** Every other table mapped in `VALID_TABLE_MAP` has an allow-list; this one was a genuine oversight — `filterTableColumns` treats a missing entry as "don't filter" (`if (!allowed || ...) return data`), so direct writes to `reports` passed through **completely unfiltered**. This corrects an incorrect claim in this document's own §2, which assumed (without verifying `mapPathToTable`, which only ever consults `VALID_TABLE_MAP`, never `TABLE_COLUMNS`) that a missing `TABLE_COLUMNS` entry caused the table to resolve to `null` and silently no-op — it does not; that append-only mistake is why this sat open through several prior passes. `reportProduct()` in `AppContext.tsx` also took `reporterId`/`reporterName` straight from local client state (`currentUser`) for its direct write, meaning a report could falsely attribute authorship to someone else, or overwrite an existing report's content outright by id.

**Fixed:** migrated `reportProduct()` to `POST /api/reports/create` — an endpoint that already existed, already `verifyUser()`-gated, and already derived `reporterId` from the verified token rather than the request body; web had simply never been switched to use it (the same "mobile/server already had it right, web hadn't migrated" pattern found repeatedly this session — except here the *unmigrated* endpoint was TedBuy's own, not mobile's). Added explicit `setDoc`/`updateDoc`/`deleteDoc` security barriers in `dbAdapter.ts` fully blocking direct writes to `reports`, the same full-closure treatment §18 applied to `notifications`' write path.

**21.3 (P2) `reviews` had a `TABLE_COLUMNS` entry but no write-ownership check on the generic path, and `addReview()` was already fully migrated (an earlier pass) to `POST /api/reviews/create` with no direct-write caller left.** A raw `setDoc(doc('reviews', existingId), {})` could still upsert-and-null an existing review's content by id (the empty-payload case that `TABLE_COLUMNS` alone can't prevent, since `id` itself would also be filtered out, leaving only the row's id set and everything else nulled by upsert). Closed with the same `setDoc`/`updateDoc`/`deleteDoc` barriers as `reports`. `POST /api/reviews/create` itself was re-verified this pass and remains fully server-authoritative: `buyerId` from `verified.uid`, `sellerId !== verified.uid` (no self-review), `chatId` required and matched via `getChatIfParticipant`, `tradeStatus === 'completed'` required (itself already protected — §14.4), `productTitle` derived from the chat rather than trusted, and a server-side duplicate-review guard. No changes needed there.

### 21.4 Tests performed (§21)

**Executed against a local dev server:**

| Test | Result |
|---|---|
| `POST /api/products/sync`, no auth, attempting `likesCount`/`viewsCount`/`likedUserIds` on an arbitrary product id | `401` ✓ |
| `POST /api/products/sync`, forged token, same payload | `401` ✓ |
| `POST /api/reports/create`, no auth | `401` ✓ |
| `POST /api/reports/create`, forged token | `401` ✓ |
| `tsc --noEmit` | Clean, 0 errors |
| `npm run build` | Succeeds, 0 errors |

Both fixed code paths sit entirely behind `verifyUser()`'s real cryptographic token verification — confirmed the fix logic itself is unreachable without a genuine Firebase identity, same standard applied throughout this document.

### 22. Broader sweep — remaining categories checked, and what's still open

Working through the full category/field list from the sweep request, beyond what §19-21 already cover:

**Re-confirmed SAFE (already fixed in earlier passes, spot-checked again this pass):** `isAdmin`/`isSuspended`/moderation `status`/`isDeleted`/`archivedAt`/`securityHold`/`isApproved` on `users` and `products` — all absent from their respective `TABLE_COLUMNS`, confirmed no legitimate client write path exists for any of them (§13/§14.1/§16). Chat `tradeStatus`/`deliveredBySeller`/`pickedUpByBuyer` — absent from `chats`' `TABLE_COLUMNS`, settable only via the seller-only/buyer-only mark-delivered/mark-picked-up endpoints (§14.4).

**Newly checked this pass, classified SAFE:**
- `role` (`users` table, `'buyer' | 'seller' | 'both'`) — a self-descriptive marketplace-activity preference, not a privilege gate; `isAdmin` is the only real privilege field and is separately protected. Legitimately client-controlled.
- `permissions`/`isModerator`/`isSuperAdmin` as literal fields — none exist server-side or in any table schema; `isSuperAdmin` client-side is always a locally-computed boolean (`email === 'asumaduvincent7@gmail.com' || isAdmin`) used only for UI gating, never persisted or trusted as an authority signal — real enforcement is server-side `verifyAdmin()`/`verified.isAdmin`, unaffected by anything client-computed.
- `featured` — not a real stored field anywhere; `/api/featured` derives featured listings via a server-side query, nothing client-settable.
- `visibility` — not a real product/business field; the only hits are the unrelated browser `document.visibilityState` API.
- Admin endpoints not yet individually confirmed in earlier passes — `/api/admin/users/search`, `/api/admin/impersonate/{start,verify,exit,logs}`, `/api/admin/users-count`, `/api/admin/accounts/deleted`, `/api/admin/retention/run-purge` — all read/traced in full. All admin-privileged operations are gated by `verifyAdmin()`/`verifyUser()+isAdmin`, except `POST /api/admin/impersonate/verify` (session-token-only, no separate admin check) and `POST /api/admin/impersonate/exit` (any authenticated user, no admin check) — both intentional and safe: `sessionId` is a 128-bit `crypto.randomBytes(16)` value (unguessable), so possession of it is itself the authentication, the same bearer-token pattern as a password-reset link. `run-purge` takes no client-influenced parameters at all — every purge criterion is server-computed from real DB state, with the existing `securityHold`/`under_investigation` check preserved.
- `sellerVerified` (product summary field) — traced and found to be a **business-logic anomaly, not a client-trust vulnerability**: `normalizeServerProductSummaryRow` computes it as `row.sellerVerified === true || row.seller_verified === true || true` — the trailing `|| true` makes the expression unconditionally `true` regardless of anything, including the row's actual value. This is entirely server-computed (no client input reaches it at all), so it's out of this sweep's scope by definition, but is flagged here since it means the "verified seller" badge currently displays for every seller unconditionally, which is presumably not the intended product behavior. **Not fixed** — the correct behavior isn't unambiguous (a real seller-verification feature may not exist yet to gate this on), and fixing product/business logic unrelated to client trust boundaries is outside this audit's mandate.

**Newly checked this pass, classified VULNERABLE but explicitly deferred (documented, not fixed):**
- `viewsCount`/`likesCount`/`likedUserIds` remain fully client-writable via `dbAdapter`'s direct-Supabase `updateDoc` path for `products` (still present in `TABLE_COLUMNS`) — `toggleLikeProduct`/`incrementProductViews` in `AppContext.tsx` depend on this for their only implementation, including anonymous (logged-out) view-tracking. Investigated `dbAdapter`'s `increment()` helper (used by `incrementProductViews`) and confirmed it provides **no actual server-side atomicity guarantee** — the "current value + delta" arithmetic happens entirely client-side, in the browser, using the RLS-less anon Supabase client, before being written as an absolute value; a caller bypassing the app's own JS convention (using raw Supabase credentials directly) can set any value with the same ease as before. This is a real residual vector for the same ranking-manipulation impact closed server-side in §21.1, but through the direct-Supabase route instead of the HTTP endpoint. **Not migrated this pass**: doing so would require new endpoint infrastructure supporting both anonymous and authenticated access (today's view-tracking works for logged-out visitors; naively requiring `verifyUser()` would regress that), which is a larger design decision rather than an unambiguous fix-in-place — deferred to sit alongside the broader RLS decision, since closing it completely requires the same category of infrastructure change RLS itself would provide.
- `notificationPreferences` (`users` table) — legitimately client-controlled for the caller's *own* row, but `dbAdapter`'s generic write path still has zero per-row ownership check, so a sufficiently sophisticated caller could set *another* user's notification preferences via raw Supabase access. Low severity (a nuisance/muting griefing vector, not privilege/money/identity), left open as part of the same RLS-dependent residual-risk bucket rather than fixed in isolation.

**No other unaudited category found.** The sweep's explicit field list (`isAdmin`, `isSuperAdmin`, `role`, `permissions`, `isSuspended`, `securityHold`, `status`, `isDeleted`, `deletedAt`, `deletionRequestedAt`, `isApproved`, `verified`/`isVerified`/`emailVerified`, `tradeStatus`, `boost*`, `payment*`, `review*`, `notification*`, `visibility`, `featured`, `archived`) has now been traced field-by-field to its actual server/client effect, across this pass and the five prior passes (§13-18) it builds on.

---

## Summary for the handoff

Nine real, distinct vulnerabilities closed across seven passes this session, all sharing the same underlying pattern (server-side or direct-Supabase trust of client-controlled privilege/financial/moderation/identity/ranking fields): `isAdmin` self-promotion (§13), `isSuspended` self-clearing (§14.1), chat `tradeStatus` fabrication enabling fraudulent reviews (§14.4), free/unlimited boost activation plus payment-reference replay (§15, the most severe), moderation-status self-reinstatement (§16), notification impersonation/phishing plus cross-user mark-read/delete (§18), username-quarantine bypass enabling identity capture of a deleted seller (§20), a `reports`-table write path left completely unfiltered by a since-corrected documentation error (§21.2), and unauthenticated-bypass-adjacent ranking manipulation via `viewsCount`/`likesCount`/`likedUserIds` (§21.1). `emailVerified` client-trust (§19) and the `reviews` table's residual write-then-null vector (§21.3) were also closed as lower-severity hardening. An unauthenticated email-abuse endpoint was also closed (§14.2), and several hundred lines of dead/redundant/insecure code removed across passes.

RLS itself remains untouched and `BLOCKED_APPROVAL` throughout every pass — none of these fixes depend on or affect that decision; each closes a vulnerability that existed independently of RLS's current state (mostly by moving trust to server-side, cryptographically-verified identity), in addition to closing the corresponding RLS-disabled direct-Supabase variant as defense-in-depth where a fix was unambiguous and didn't require new infrastructure. A small number of narrower, direct-Supabase-only residual risks remain explicitly documented rather than fixed (§19's `emailVerified` self-mirroring, §22's `viewsCount`/`likesCount`/`likedUserIds` and `notificationPreferences`) — each judged lower-severity, each requiring either new endpoint infrastructure or a product-design decision beyond an unambiguous fix-in-place, and each belonging to the same root-cause bucket (RLS disabled + no per-row ownership check in `dbAdapter.ts`'s generic path) that the pending RLS migration decision is meant to close comprehensively rather than field-by-field indefinitely.
