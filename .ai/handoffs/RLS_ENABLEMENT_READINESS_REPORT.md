...
0. **New prerequisite, not in the original plan: migrate `server.ts` off the anon key onto a real `service_role` key.** Get the `service_role` key from the Supabase dashboard (Project Settings → API — it's a separate, more privileged JWT than the anon key, never exposed to any client, only ever used server-side), add it as a new env var (e.g. `SUPABASE_SERVICE_ROLE_KEY`), and change `server.ts`'s `backendSupabase` initialization (currently reading `VITE_SUPABASE_ANON_KEY`/`SUPABASE_ANON_KEY`) to use it instead. Verify the app still works end-to-end afterward — this swaps the server's credential without changing any query logic, so it should be low-risk on its own, but it's a real code + config change touching every single endpoint's database access and deserves its own careful, isolated rollout and verification before RLS enters the picture at all. **Nothing past this point is safe to do until this step is complete and verified.**
