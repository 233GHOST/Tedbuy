...
**STATUS: IMPLEMENTATION REVIEW APPROVED (2026-09-18) — NO DATABASE OR CODE CHANGES MADE.** Builds on the shape already approved in `ADMIN_AUDIT_LOGS_SCHEMA_PROPOSAL.md` (commit `6ba4733`). All three points flagged in the first pass of this review are now approved by Vincent: (1) the `verifyAdmin()` → `verifyUser()` swap at boost-control and retention-purge, solely to obtain the UID, proven behavior-preserving in §5.8; (2) `impersonate_start`'s protected/nonexistent-target rejections get `result: 'failure'` rows, scoped to only the branches that reach a real, identified admin and a real, identified target — not every possible rejection in the handler; (3) every other identity-sourcing and design decision in this document stands as originally reviewed. Still no `CREATE TABLE`, no `ALTER TABLE`, no RLS/policy/grant change, no application code change, no deployment — this remains a documentation-only update reflecting the sign-off, not an implementation pass.
...
**Current**: calls `logImpersonationEvent({ adminUserId: verifiedUser.uid, ... })` — `verifiedUser` here already comes from a real `verifyUser()` call earlier in this same handler (confirmed, §6). This endpoint's realistic failure modes (target not found, self-impersonation blocked, super-admin-impersonation blocked) all `return` *before* reaching this call today — a failed impersonation attempt currently logs nothing.

**Approved (2026-09-18)**: rejected attempts — including protected/nonexistent targets — produce a `result: 'failure'` audit record, but **only from the point the request reaches the real administrative action**, not for every possible rejection in this handler. Concretely: `verifyAdmin()` failing (`403`, not a real admin at all) and a malformed request body (`400`, no `targetUserId`/`targetUserEmail`) happen *before* there's a cryptographically-verified actor and a resolved target to attribute a row to — consistent with every other action type in this table (§7's own stated rule: nothing meaningful to log before identity is established). The two rejection branches that **do** reach a real, identified admin attempting a real, identified action — "target is the protected super-admin" and "target does not exist" — are the ones that get a `result: 'failure'` row, since at that point `verified.uid` (the real admin) and a specific target are both already known.

**Proposed**: (1) add `ipAddress`/`userAgent` from `req` to every call; (2) add a `logImpersonationEvent()` call (or equivalent) on the "target is protected" and "target not found" branches specifically, with `result: 'failure'`, `error_message` set to a short, non-sensitive reason (`'target account is protected'` / `'target account not found'` — never echoing back raw request internals or stack traces, per §9's metadata-hygiene rule), `metadata` empty or limited to the same non-sensitive reason. `target_user_id` is populated when the target was at least identifiable (the "not found" case may only have the raw id/email the admin typed, not a real resolved user — still worth recording as `target_user_id` if that's what was supplied, since it's exactly the security-relevant detail an investigator would want: which id did the admin attempt to target).
...
**Approved (2026-09-18)**: switching from `verifyAdmin()` to `verifyUser()` here is approved specifically to obtain the cryptographically-verified UID for audit logging — not for any other reason, and not to change what this endpoint otherwise does.

**Proposed**: (1) switch the auth call from `verifyAdmin(req.headers.authorization)` to `verifyUser(req.headers.authorization)`, checking `verified?.isAdmin || verified?.originalAdmin` for the *exact same authorization decision as today* — `verifyAdmin()` itself (`server.ts:328-331`) is nothing more than `!!user?.isAdmin || !!user?.originalAdmin` computed from a `verifyUser()` call internally, so this swap is provably behavior-preserving: the two checks are definitionally identical, just with one extra step exposing the `uid`/`email` this table needs. No admin who currently passes `verifyAdmin()` would ever fail the equivalent `verifyUser()` + inline check, and vice versa. This matches the exact pattern already live at security-hold/suspend/delete (§5.5-5.7), not a new one; (2) after the real Supabase/Firestore write's outcome is known, insert a row with `target_product_id: productId`, `action: boost_admin_activate | boost_admin_deactivate`, `metadata: { planId, durationDays, boostEndDate }` for activate, `metadata: {}` for deactivate.
...
**Approved (2026-09-18)**: same approval and reasoning as §5.8 — the `verifyAdmin()` → `verifyUser()` swap is provably behavior-preserving (§5.8's proof applies identically here, same two functions, same call shape), approved solely to obtain the UID for audit logging.
...
**Nothing here has been executed.** This document, including the three approvals recorded above, is the exact-SQL/exact-code review requested. All of it — the schema, the RLS approach, the 11 action values, all 10 call sites, the failure-logging design, IP/UA capture, the read endpoint design, and now the three previously-open points — is approved as a *specification*. Writing it as a real diff, and Vincent running the SQL, are still separate, not-yet-taken next steps.
