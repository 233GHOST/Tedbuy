...
if (backendSupabase) {
  if (isServiceRoleUsed) {
    ...
    console.warn('[Supabase Server] WARNING: SUPABASE_SERVICE_ROLE_KEY is not configured. Falling back to VITE_SUPABASE_ANON_KEY. Server-mediated operations for protected tables (chats, messages, notifications, reviews, etc.) WILL FAIL if Row Level Security (RLS) is enabled!');
  }
} else {
...
`);
  ...
  // ------------------------------------------------------------------
  // SECURE SERVER-MEDIATED ENDPOINTS FOR PRIVATE DATA
  // ------------------------------------------------------------------
  app.get('/api/chats', serverRateLimiter(60 * 1000, 150, "chats-list"), async (req, res) => {
    try {
      ...
      if (!backendSupabase) {
        return res.status(503).json({ success: false, error: 'Supabase service is unavailable' });
      }
      ...
      console.error('[GET /api/chats] Error fetching chats:', err);
      return res.status(500).json({ success: false, error: err.message || 'Internal server error' });
    }
  });

  app.get('/api/chats/:chatId/messages', serverRateLimiter(60 * 1000, 250, "chat-messages"), async (req, res) => {
    try {
      const user = await verifyUser(req.headers.authorization);
      if (!user) {
      ...
      // 1. Verify user is a participant in the chat
      const { data: chat, error: chatErr } = await backendSupabase
        .from('chats')
        .select('*')
        .eq('id', chatId)
        .maybeSingle();
      ...
      // 2. Fetch messages
      const { data: messages, error: msgErr } = await backendSupabase
        .from('messages')
        .select('*')
        .eq('chatId', chatId)
      ...
      console.error('[GET /api/chats/:chatId/messages] Error fetching messages:', err);
      return res.status(500).json({ success: false, error: err.message || 'Internal server error' });
    }
  });

  app.post('/api/chats/:chatId/messages', serverRateLimiter(60 * 1000, 150, "send-message"), async (req, res) => {
    try {
      const user = await verifyUser(req.headers.authorization);
      if (!user) {
        return res.status(401).json({ success: false, error: 'Unauthorized' });
      ...
      // 2. Determine and verify recipient
      const computedRecipientId = user.uid === chat.buyerId ? chat.sellerId : chat.buyerId;
      ...
      // 3. Insert message
      const { error: insertErr } = await backendSupabase
        .from('messages')
        .insert(newMessage);
      ...
      // 4. Update parent chat's metadata
      ...
      console.error('[POST /api/chats/:chatId/messages] Error creating message:', err);
      return res.status(500).json({ success: false, error: err.message || 'Internal server error' });
    }
  });

  app.post('/api/chats', serverRateLimiter(60 * 1000, 100, "chats-create"), async (req, res) => {
    try {
      const user = await verifyUser(req.headers.authorization);
      if (!user) {
        return res.status(401).json({ success: false, error: 'Unauthorized' });
      ...
      const chatId = id || `chat_${user.uid}_${sellerId}_${productId}_${Date.now()}`;

      // Get the buyerName if not supplied
      let finalBuyerName = buyerName;
      if (!finalBuyerName) {
        const { data: userData } = await backendSupabase
          .from('users')
          .select('username')
          .eq('id', user.uid)
          .maybeSingle();
        finalBuyerName = userData?.username || user.email.split('@')[0] || 'Buyer';
      }
      ...
      const newChat = {
        ...
        lastMessageText: lastMessageText || 'Chat started',
      ...
      console.error('[POST /api/chats] Error creating chat:', err);
      return res.status(500).json({ success: false, error: err.message || 'Internal server error' });
    }
  });

  app.patch('/api/messages/:messageId/read', serverRateLimiter(60 * 1000, 200, "messages-read"), async (req, res) => {
    try {
      const user = await verifyUser(req.headers.authorization);
      if (!user) {
        return res.status(401).json({ success: false, error: 'Unauthorized' });
      ...
      // 1. Fetch message to check recipientId
      const { data: message, error: fetchErr } = await backendSupabase
        .from('messages')
        .select('*')
        .eq('id', messageId)
        .maybeSingle();
      ...
      if (message.recipientId !== user.uid) {
        return res.status(403).json({ success: false, error: 'Forbidden: You cannot mark this message as read' });
      }
      ...
      console.error('[PATCH /api/messages/:messageId/read] Error marking read:', err);
      return res.status(500).json({ success: false, error: err.message || 'Internal server error' });
    }
  });

  app.get('/api/notifications', serverRateLimiter(60 * 1000, 150, "notifications-list"), async (req, res) => {
    try {
      const user = await verifyUser(req.headers.authorization);
      if (!user) {
        return res.status(401).json({ success: false, error: 'Unauthorized' });
  ...
  app.patch('/api/notifications/:notificationId/read', serverRateLimiter(60 * 1000, 200, "notifications-read"), async (req, res) => {
    try {
      const user = await verifyUser(req.headers.authorization);
      if (!user) {
        return res.status(401).json({ success: false, error: 'Unauthorized' });
      ...
      // 1. Fetch notification to check userId
      const { data: notification, error: fetchErr } = await backendSupabase
        .from('notifications')
        .select('*')
      ...
      console.error('[PATCH /api/notifications/:notificationId/read] Error marking notification as read:', err);
      return res.status(500).json({ success: false, error: err.message || 'Internal server error' });
    }
  });

  app.post('/api/reviews', serverRateLimiter(60 * 1000, 50, "reviews-create"), async (req, res) => {
    try {
      const user = await verifyUser(req.headers.authorization);
      if (!user) {
        return res.status(401).json({ success: false, error: 'Unauthorized' });
      ...
      if (!sellerId) {
        return res.status(400).json({ success: false, error: 'Missing seller ID' });
      }

      const reviewId = id || `rev_${Date.now()}_${Math.random().toString(36).substring(2, 11)}`;
      ...
      console.error('[POST /api/reviews] Error creating review:', err);
      return res.status(500).json({ success: false, error: err.message || 'Internal server error' });
    }
  });
