...
`);
  ...
  // Bounds any single awaited call so a slow/hung dependency (Firestore, Supabase, etc.)
  // can never freeze the whole request indefinitely -- it just fails safe instead.
  ...
  async function verifyAdmin(authHeader: string | undefined): Promise<boolean> {
    ...
    try {
      ...
      if (getApps().length > 0) {
        ...
        const decoded = await withTimeout(getAdminAuth().verifyIdToken(token), 4000, null as any);
        if (!decoded) {
          console.warn('[verifyAdmin] Firebase token verification timed out, proceeding to fallback.');
        } else {
          if (decoded.email?.trim()?.toLowerCase() === 'asumaduvincent7@gmail.com') {
            return true;
          }
          if (adminDb) {
            const userSnap = await withTimeout(adminDb.collection('users').doc(decoded.uid).get(), 4000, null as any);
            if (userSnap && userSnap.exists && userSnap.data()?.isAdmin === true) {
              return true;
            }
          }
          if (backendSupabase) {
            const { data, error } = await withTimeout(
              backendSupabase.from('users').select('is_admin').eq('id', decoded.uid).maybeSingle(),
              4000,
              { data: null, error: new Error('timed out') } as any
            );
    ...
    try {
      ...
      if (parts.length >= 2) {
        ...
        if (uid) {
          if (adminDb) {
            const userSnap = await withTimeout(adminDb.collection('users').doc(uid).get(), 4000, null as any);
            if (userSnap && userSnap.exists && userSnap.data()?.isAdmin === true) {
              return true;
            }
          }
  ...
  async function verifyUser(authHeader: string | undefined): Promise<{ uid: string; email: string } | null> {
    ...
    try {
      ...
      if (getApps().length > 0) {
        ...
        if (!decoded) {
          console.warn('[verifyUser] Firebase token verification timed out, proceeding to fallback.');
        }
      }
    } catch (err) {
