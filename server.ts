...
<body>
  ...
  app.post('/api/auth/reset-password', serverRateLimiter(5 * 60 * 1000, 3, "auth-reset-password"), async (req, res) => {
    ...
    try {
      ...
      if (resetLink) {
        const originalLink = resetLink;
        // Rewrite the default firebaseapp.com / web.app domain to tedbuy.store
        resetLink = resetLink.replace(/https:\/\/[a-zA-Z0-9-]+\.firebaseapp\.com/g, "https://tedbuy.store");
        resetLink = resetLink.replace(/https:\/\/[a-zA-Z0-9-]+\.web\.app/g, "https://tedbuy.store");
        console.log(`[Auth Reset] Password reset link rewritten from ${originalLink} to ${resetLink}`);
      }
