...
// Prevents a "cache stampede": when multiple requests arrive while the cache is cold,
// only one Firestore fetch runs and everyone else waits on the same promise instead of
// each independently pulling all 300 products into memory at once.
...
try {
  if (fs.existsSync(CACHE_FILE_PATH)) {
    ...
    // Timestamp is deliberately backdated past CACHE_TTL_MS so this placeholder reads as
    // already-expired -- otherwise the very first request after every deploy would see a
    // "fresh" empty cache and serve 0 products for a full 5 minutes before ever touching Firestore.
    fs.writeFileSync(CACHE_FILE_PATH, JSON.stringify({ data: [], timestamp: 0 }), 'utf-8');
    cachedProducts = {
      data: [],
      timestamp: 0
    };
    console.log(`[Products Cache] Initialized empty file cache (pre-expired, will fetch fresh data on first request).`);
  }
} catch (cacheErr) {
...
`);
  ...
  app.get('/api/products', serverRateLimiter(60 * 1000, 120, "products-list"), async (req, res) => {
    try {
      ...
      // If another request already kicked off a fetch, wait on that same promise instead
      // of starting a second (or tenth) parallel Firestore query.
      if (!inFlightProductsFetch) {
        inFlightProductsFetch = refreshProductsCache().finally(() => {
      ...
      if (cachedProducts) {
        res.setHeader('Cache-Control', 'public, max-age=5, stale-while-revalidate=15');
        return res.json({ success: true, products: cachedProducts.data });
      }
  ...
  // Fetches products from Firestore (Admin SDK, falling back to REST) and updates cachedProducts.
  // Never throws for expected failure modes (429, permission errors, etc.) -- it logs and leaves
  // cachedProducts as-is so callers can fall back to stale/file cache. Only truly unexpected
  // exceptions propagate, and even those are caught by the route's own catch block.
  async function refreshProductsCache(): Promise<void> {
    try {
      ...
      // Deliberately not re-thrown: leaving cachedProducts as whatever we recovered
      // (or null) lets the route handler decide what to serve.
    }
  }
