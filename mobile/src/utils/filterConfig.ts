/** Direct port of web's src/utils/filterConfig.ts — same category → extra
 * filter field mapping and brand→model data, verbatim. Mobile previously had
 * no category-specific filtering at all (just a generic min/max price
 * filter for every category, and a selectedBrandFilter that no UI ever set),
 * while this is a real, working feature on web (gates product-list inclusion
 * via productSelector.ts's matchesExtra check, not cosmetic). */
  ...
  'Furniture & Home': [
    {
      ...
      type: 'select',
      options: ['Sofas & Living Room', 'Beds & Bedroom Sets', 'Dining Tables & Chairs', 'Office Furniture', 'Wardrobes & Storage', 'Decor, Lighting & Rugs', 'Kitchen & Outdoor']
    },
    {
      id: 'material',
      label: 'Primary Material',
      type: 'select',
      options: ['Solid Wood', 'Leather', 'Metal & Steel', 'Glass', 'Fabric / Upholstered', 'Plastic / Synthetic']
    },
    {
      id: 'condition',
      label: 'Condition',
      type: 'select',
      options: ['Brand New', 'Slightly Used (Home Used)', 'Ghana Used']
    }
  ],
  'Jobs & Employment': [
    {
      ...
      label: 'Job Sector',
      type: 'select',
      options: ['Accounting & Finance', 'Customer Care & Admin', 'IT & Software Engineering', 'Sales & Marketing', 'Teaching & Education', 'Driver & Transport', 'Hospitality & Cleaning', 'Trades & Construction']
    },
    {
      id: 'employmentType',
      label: 'Employment Type',
      type: 'select',
      options: ['Full-Time', 'Part-Time', 'Contract', 'Remote / Work from Home', 'Internship']
    },
    {
      id: 'salaryPeriod',
      label: 'Salary Frequency',
      type: 'select',
      options: ['Monthly', 'Weekly', 'Daily / Hourly', 'Commission / Project']
    }
  ],
  'Agriculture & Food': [
    {
      ...
      label: 'Category Type',
      type: 'select',
      options: ['Fresh Produce & Crops', 'Livestock & Poultry', 'Seeds & Fertilizers', 'Farm Machinery & Tools', 'Animal Feed', 'Fresh Food & Spices']
    },
    {
      id: 'condition',
      label: 'Supply Format',
      type: 'select',
      options: ['Fresh Harvest', 'Processed / Packaged', 'Wholesale / Bulk', 'Retail Unit']
    }
  ],
  'Pets & Animals': [
    {
      id: 'animalCategory',
      label: 'Animal / Product Type',
      type: 'select',
      options: ['Dogs & Puppies', 'Cats & Kittens', 'Birds & Parrots', 'Fish & Aquariums', 'Livestock & Farm Animals', 'Pet Food & Accessories']
    },
    {
  ...
  'Sports & Fitness': [
    {
      ...
      type: 'select',
      options: ['Gym & Weight Training', 'Bicycles & Cycling', 'Football & Team Sports', 'Swimming & Water Sports', 'Outdoor & Camping', 'Martial Arts & Boxing']
    },
    {
      id: 'condition',
      label: 'Condition',
  ...
  'Kids & Baby': [
    {
      ...
      label: 'Item Category',
      type: 'select',
      options: ['Baby Clothing & Shoes', 'Strollers & Car Seats', 'Toys & Learning Games', 'Baby Feeding & Care', 'Kids Furniture & Cribs', 'Maternity Products']
    },
    {
      id: 'ageGroup',
      label: 'Age Group',
      type: 'select',
      options: ['0 - 6 Months', '6 - 12 Months', '1 - 3 Years', '4 - 7 Years', '8+ Years']
    }
  ],
  'Commercial & Tools': [
    {
      ...
      type: 'select',
      options: ['Power & Hand Tools', 'Industrial Machinery', 'Generators & Solar Systems', 'Construction Equipment', 'Restaurant & Store Fixtures', 'Medical & Dental Supplies']
    },
    {
      id: 'condition',
      label: 'Condition',
  ...
  'Books & Hobbies': [
    {
      ...
      label: 'Item Type',
      type: 'select',
      options: ['Educational & Textbooks', 'Novels & Fiction', 'Musical Instruments', 'Art & Craft Supplies', 'Board Games & Puzzles', 'Collectibles & Antiques']
    },
    {
      id: 'condition',
      label: 'Condition',
      type: 'select',
      options: ['Brand New', 'Used - Like New', 'Used - Good']
    }
  ],
  Other: [
    {
...
export function getModelsForBrand(brand: string, category: string): string[] {
  if (!brand) return [];
