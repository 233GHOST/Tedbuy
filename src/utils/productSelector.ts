import { Product, Category, User, normalizeCategory } from '../types';
import { getRegionForLocation } from '../regions';
import { parseDate, isBoostActive } from './dateParser';
  ...
  return (
    ...
    const filtered = products.filter(product => {
      ...
      const matchesCategory = !category || 
        ...
        (prodCategory && category && prodCategory.toLowerCase() === (category as string).toLowerCase()) ||
        normalizeCategory(prodCategory) === normalizeCategory(category);
      ...
      const matchesSearch = !queryText.trim() ||
        ...
        normalizeCategory(prodCategory).toLowerCase().includes(queryText.toLowerCase()) ||
        prodDesc.toLowerCase().includes(queryText.toLowerCase()) ||
        prodLocation.toLowerCase().includes(queryText.toLowerCase()) ||
        (product.brand && String(product.brand).toLowerCase().includes(queryText.toLowerCase()));
    ...
    const sorted = [...filtered].sort((a, b) => {
      ...
      if (boostA && boostB) {
        ...
        // Package hierarchy: GH₵10 (1 Month) > GH₵7 (21 Days) > GH₵5 (14 Days) > GH₵3 (7 Days) > GH₵1 (3 Days)
        const getBoostPriorityLevel = (planId?: string): number => {
          if (!planId) return 0;
          if (planId === '1month' || planId === '90days') return 5; // GH₵10
          if (planId === '21days' || planId === '30days') return 4; // GH₵7
      ...
      // Under explicit price sorting, sort primarily by price and fallback to date/rankingScore for tie breaking
      if (sortByPrice === 'asc') {
        const diff = parseNumericPrice(a.price) - parseNumericPrice(b.price);
        if (diff !== 0) return diff;
      } else if (sortByPrice === 'desc') {
      ...
      // Sort by Ads Creation Date (Newest First vs Oldest First)
      const pDateA = parseDate(a.createdAt);
      const pDateB = parseDate(b.createdAt);
      const dateA = pDateA ? pDateA.getTime() : 0;
      const dateB = pDateB ? pDateB.getTime() : 0;
      ...
      // Fallback: sort by smart rankingScore if dates are identical
      if (scoresA.rankingScore !== scoresB.rankingScore) {
        return scoresB.rankingScore - scoresA.rankingScore;
      }
