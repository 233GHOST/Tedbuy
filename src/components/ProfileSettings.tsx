...
CEO, Tedbuy Inc`;
  ...
  const handleRemoveBookmark = async (productId: string, e?: React.MouseEvent) => {
    ...
    try {
      ...
      let msg = 'Failed to update bookmarks';
      if (err instanceof Error) {
        try {
          const parsed = JSON.parse(err.message);
          if (parsed.error) msg = parsed.error;
                                  ...
                                  <div className="sm:text-right shrink-0 flex items-center justify-end gap-2 flex-wrap sm:flex-nowrap">
                                    ...
                                    {isSuperAdmin ? (
                                      ...
                                      <>
                                        ...
                                        {/* Wires up the existing (previously unreachable) delete-profile
                                            confirmation dialog/handler below -- adminDeleteUserProfile and
                                            its dialog were fully implemented but had no button anywhere in
                                            the app that could open them, making permanent account deletion
                                            unreachable from the UI entirely. */}
                                        <button
                                          type="button"
                                          disabled={isSelf || adminDeletingId === u.id}
                                          onClick={() => setActiveAccountConfirmUser({ id: u.id, username: u.username || 'Anonymous User' })}
                                          title={isSelf ? "You can't delete your own account from here" : 'Permanently delete this store profile'}
                                          className="w-full sm:w-auto px-3 py-2 text-[10px] font-black uppercase tracking-wider rounded-xl cursor-pointer select-none transition duration-150 flex items-center justify-center gap-1.5 bg-rose-950/60 hover:bg-rose-900/70 border border-rose-800/60 hover:border-rose-700 text-rose-300 disabled:opacity-40 disabled:cursor-not-allowed"
                                        >
                                          {adminDeletingId === u.id ? (
                      ...
                      <button
                        ...
                        onClick={async () => {
                          try {
                            ...
                            // Previously console.error only -- unfollowSeller throws via
                            // handleBackendError on any real failure (network, expired
                            // session, server error), which this swallowed with zero
                            // user feedback: no toast, no state change, nothing to show
                            // whether the click did anything at all.
                            console.error(err);
                            let msg = 'Could not unfollow this store. Please try again.';
                            if (err instanceof Error) {
                              try {
                                const parsed = JSON.parse(err.message);
                                if (parsed.error) msg = parsed.error;
                          ...
                          <button
                            ...
                            onClick={async () => {
                              try {
                                ...
                                console.error(err);
                                let msg = 'Could not follow this store. Please try again.';
                                if (err instanceof Error) {
                                  try {
                                    const parsed = JSON.parse(err.message);
                                    if (parsed.error) msg = parsed.error;
