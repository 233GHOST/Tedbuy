...
import { ArrowLeft, MessageSquare, MapPin, Eye, Calendar, UserPlus, UserCheck, ChevronRight, ShieldAlert, Bookmark, X, Camera, ChevronLeft, Maximize2, Edit2, Trash2, Share2, Check, Package, RefreshCw, Plus, Sparkles, Video, Loader2, Flame } from 'lucide-react';
import { ProductCard } from './ProductCard';
import { ListingModal } from './ListingModal';
import { isUserVerified, calculateTrustScore, normalizeCategory } from '../types';
import { slugify } from '../utils/slugify';
import { auth } from '../firebase';
import { isBoostActive, parseDate, getBoostEndDate } from '../utils/dateParser';
import { getOptimizedImageUrl } from '../utils/imageOptimizer';
import { resolveProductImage, resolveProductImages, getCategoryPlaceholder } from '../utils/productUtils';
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 text-left">
        ...
        <div className="lg:col-span-5 space-y-6">
          <div className="bg-white border border-slate-200 p-6 rounded-3xl shadow-xs space-y-5">
            <div className="space-y-2">
              <div className="flex items-center gap-2.5 flex-wrap">
                ...
                {isBoostActive(product) && (
                  <span className="px-2.5 py-1 bg-amber-500 text-slate-950 text-[10px] font-black rounded-lg flex items-center gap-1 uppercase tracking-wider shadow-sm animate-pulse" title="Active Seller Boosted Listing">
            ...
            <div className={`grid ${isServiceCategory ? 'grid-cols-2' : 'grid-cols-3'} gap-2 bg-slate-50 border border-slate-150 rounded-2xl p-3 font-sans`}>
              <div>
                                ...
                                onClick={async () => {
                                  ...
                                  try {
                                    ...
                                    if (res.ok && data.success) {
                                      ...
                                      console.warn('Backend boost-control API rejected request, falling back to direct backend update...', data.error);
                                      await handleDeactivateBoostSilently();
                                      setShowDeactivateConfirm(false);
                                    }
                                  } catch (err: any) {
                                    console.error('Error contacting boost-control API, falling back to direct backend update...', err);
                                    await handleDeactivateBoostSilently();
                                    setShowDeactivateConfirm(false);
                                  } finally {
                                    setIsAdminBoosting(false);
                          ...
                          onClick={async () => {
                            ...
                            try {
                              ...
                              if (res.ok && data.success) {
                                ...
                                if (data.product) {
                                  // Update the local react state only since the backend already successfully updated the record
                                  await updateProduct(product.id, data.product, true);
                                }
                              } else {
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="flex-1 text-left min-w-0">
                <h4 id="detail-seller-name" className="text-sm font-bold text-slate-900 flex items-center gap-1.5 min-w-0 flex-wrap">
                  ...
                  {isBoostActive(product) && (
                    <span className="inline-flex items-center gap-1 text-[9px] text-slate-950 font-black bg-amber-500 border border-amber-600 px-1.5 py-0.5 rounded-md shrink-0 shadow-xs" title="Active Seller Boosted Ad">
                      <Flame className="w-2.5 h-2.5 text-slate-950 fill-slate-950 animate-bounce" /> Active Seller
                    </span>
                  )}
                </h4>
