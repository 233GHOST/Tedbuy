  ...
  const mediaGallery = product ? [
   ...
   const productImageUrl = product?.images?.[0]?.startsWith('data:')
     ? typeof window !== 'undefined'
       ...
       : `https://ghanamarketplace.com/api/products/${product.id}/image.jpg`
    ...
    if (navigator.share) {
      try {
        await navigator.share({
          ...
          text: `Check out "${product?.title || 'this item'}" on Tedbuy Ghana! Price: ${product?.price ? 'GHS ' + product?.price : 'Negotiable'}. View details:`,
          url: shareUrl,
        });
      } catch (err) {
            ...
            <div className="border-t border-slate-100 pt-5 mt-2 space-y-4">
              ...
              <div className="bg-slate-50 rounded-2xl p-4 border border-slate-200 mt-2 space-y-3">
                <div className="flex items-center justify-between border-b border-slate-200/60 pb-2">
                  <span className="text-[11px] font-bold text-slate-505 uppercase tracking-wider flex items-center gap-1">
                    <span>📱</span>
                    <span>Social Preview Simulator</span>
                  </span>
                  <div className="flex gap-1">
                    {(['whatsapp', 'facebook', 'tiktok'] as const).map((mode) => (
                      <button
                        key={mode}
                        onClick={() => setPreviewPlatform(mode)}
                        className={`px-2 py-0.5 rounded-md text-[10px] font-bold uppercase tracking-tight transition ${
                      ...
                      >
                        {mode === 'tiktok' ? 'TikTok' : mode === 'whatsapp' ? 'WhatsApp' : 'Facebook'}
                      </button>
                    ))}
                  </div>
                </div>
                ...
                {previewPlatform === 'whatsapp' && (
                  <div className="bg-emerald-50 rounded-xl p-3 max-w-[340px] mx-auto shadow-xs">
                    <div className="bg-[#DCF8C6] text-[#303030] text-xs p-2.5 rounded-xl border border-[#C6E1A4] leading-relaxed relative text-left">
                      ...
                      <div className="bg-[#E4F5D4] rounded-lg border border-[#D0E6BE] overflow-hidden mb-1 flex flex-col cursor-pointer hover:bg-[#DCF1CC] transition">
                        ...
                        <div className="p-2 space-y-0.5 border-t border-[#D0E6BE]/60 text-[#128C7E]">
                          <div className="font-bold text-[12px] line-clamp-1">
                            {product?.title || 'Product Title'} - {product?.price ? `GHS ${product.price}` : 'Negotiable'}
                          </div>
                          <div className="text-[11px] text-[#2c3e50] line-clamp-2 leading-tight">
                            {product?.description || 'No description provided.'}
                          </div>
                          <div className="text-[10px] text-slate-500 uppercase tracking-widest font-semibold pt-0.5">
                            {typeof window !== 'undefined' ? window.location.hostname : 'tedbuy-fb79a.web.app'}
                          </div>
                        </div>
                      </div>
                      <p className="text-slate-800 text-[11px] leading-tight break-all">
                        Check out "{product?.title || 'this item'}" on Tedbuy Ghana! Price: {product?.price ? 'GHS ' + product?.price : 'Negotiable'}. View details: <span className="text-blue-600 font-medium underline select-all">{shareUrl.slice(0, 45)}...</span>
                      </p>
                      <div className="text-[8px] text-[#128C7E]/70 text-right mt-0.5 select-none">
                        delivered ✓✓
                      </div>
                    </div>
                  </div>
                ...
                {previewPlatform === 'facebook' && (
                  <div className="bg-white border border-slate-200 rounded-xl max-w-[340px] mx-auto shadow-sm overflow-hidden text-slate-900 text-left">
                    <div className="flex items-center gap-2 p-2">
                      <div className="w-7 h-7 rounded-full bg-blue-600 text-white flex items-center justify-center font-bold text-xs">
                        TB
                      </div>
                      <div>
                        <div className="text-[11px] font-bold font-sans">TedBuy Ghana Classifieds</div>
                    ...
                    <div className="px-2 pb-1.5 text-[11px] leading-snug">
                    ...
                    <div className="bg-slate-100 border-t border-slate-200 cursor-pointer hover:bg-slate-150 transition">
                      ...
                      <div className="p-2 bg-white border-t border-slate-100 flex flex-col space-y-0.5">
                        <span className="text-[9px] uppercase text-slate-500 tracking-wider font-semibold">
                          {typeof window !== 'undefined' ? window.location.hostname : 'tedbuy-fb79a.web.app'}
                        </span>
                        ...
                        <span className="text-[10px] text-slate-500 line-clamp-1">
                          {product?.description || 'Browse high-quality local Ghana items.'}
                        </span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="bg-slate-950 text-white rounded-xl max-w-[340px] mx-auto overflow-hidden relative shadow-md font-sans border border-slate-800 text-left">
                    <div className="aspect-[1.91/1] w-full bg-slate-900 relative flex items-center justify-center overflow-hidden">
                      {productImageUrl ? (
                        ...
                        <div className="text-slate-500 text-xs text-center w-full py-4">No media available</div>
                      )}
                      <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-transparent to-black/20 pointer-events-none" />
                      
                      <div className="absolute bottom-2 left-2 right-12 text-white space-y-0.5">
                        <div className="font-bold text-[10px] tracking-tight flex items-center gap-1">
                          <span className="text-pink-500">@tedbuy.ghana</span>
                          <span className="text-[7px] bg-sky-505 px-1 py-0.2 rounded-full font-bold">Follow</span>
                        </div>
                        <p className="text-[9px] leading-snug text-slate-100 line-clamp-2">
                          ⚡ Check out <span className="font-bold text-yellow-400">"{product?.title}"</span> on Tedbuy Ghana! Price: {product?.price ? 'GHS ' + product?.price : 'Negotiable'}. Listing Link is pasted directly on our bio! #tedbuy #ghana
                        </p>
                      </div>

                      <div className="absolute right-1 bottom-1 flex flex-col items-center gap-1 text-[8px] font-semibold text-white">
                        <div className="w-5 h-5 rounded-full bg-slate-800/80 border border-slate-700 flex items-center justify-center text-[7px] font-bold text-pink-500 shadow-lg">
                          TB
                        </div>
              ...
              <div className="flex flex-wrap gap-2 pt-1">
                {/* WhatsApp */}
                <a
                  href={`https://api.whatsapp.com/send?text=${encodeURIComponent(`Check out "${product.title}" on Tedbuy Ghana! Price: ${product.price ? 'GHS ' + product.price : 'Negotiable'}. View details: ` + shareUrl)}`}
                  target="_blank"
                ...
                {/* Telegram */}
                <a
                  href={`https://t.me/share/url?url=${encodeURIComponent(shareUrl)}&text=${encodeURIComponent(`Check out "${product.title}" on Tedbuy Ghana! Price: ${product.price ? 'GHS ' + product.price : 'Negotiable'}. View details:`)}`}
                  target="_blank"
                  rel="noopener noreferrer"
                ...
                {/* Twitter / X */}
                <a
                  href={`https://twitter.com/intent/tweet?url=${encodeURIComponent(shareUrl)}&text=${encodeURIComponent(`Check out "${product.title}" on Tedbuy Ghana! Price: ${product.price ? 'GHS ' + product.price : 'Negotiable'}. View details:`)}`}
                  target="_blank"
                  rel="noopener noreferrer"
