...
export const ProductCard: React.FC<ProductCardProps> = ({ product }) => {
  ...
  // Memoize seller lookup to avoid O(N) searching of users list on every rendering
  const seller = React.useMemo(() => {
    return users?.find(u => u.id === product.sellerId);
  }, [users, product.sellerId]);

  const isSellerVerified = React.useMemo(() => isUserVerified(seller), [seller]);

  const handleDetailsClick = React.useCallback(() => {
    setSelectedProductId(product.id);
    setCurrentView('product-detail');
  }, [product.id, setSelectedProductId, setCurrentView]);
  ...
  }, [currentUser, product.id, toggleSaveProduct, setAuthMode, setShowAuthModal]);

  const formattedPrice = React.useMemo(() => formatProductPrice(product.price), [product.price]);
  ...
  }, [product, isSaved, isSellerVerified, formattedPrice, dateFormatted, handleDetailsClick, handleSaveClick]);
};
