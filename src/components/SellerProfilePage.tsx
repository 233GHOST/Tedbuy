...
export const SellerProfilePage: React.FC = () => {
  ...
  // Security fix (RLS-migration Phase 0): `users` (the shared directory
  // state above) now comes from the PII-safe GET /api/users/list, which
  // deliberately omits contact info since it returns everyone in one
  // response. The "Contact via WhatsApp" feature below needs this one
  // specific seller's phoneNumber/whatsAppNumber -- data a seller
  // publishes specifically so buyers can reach them, unlike the rest of
  // the (former) bulk PII exposure this migration closes -- so it's
  // fetched as its own single, targeted lookup via the existing
  // GET /api/users/get, which already includes contact info for exactly
  // this reason (one user at a time, not a scrapable directory).
  ...
  // free-choice/any-listing dropdown — see eligibleReviewChats below.
  ...
  // Reviews are only authentic if tied to an actual completed trade with
  // this exact seller — previously ANY logged-in visitor could tap "Rate
  // Seller" here and post a review with an arbitrary/optional product tag,
  // with nothing behind it. Restricted to the buyer's own chats with this
  // seller that have actually reached tradeStatus:'completed' (see
  // markAsDelivered/markAsPickedUp), matching exactly what ChatInterface's
  // in-chat "Leave Review" button already requires — this just extends the
  // same rule to this entry point, which previously had none. Also filters
  // out trades already reviewed, mirroring the server's own
  // buyerId+sellerId+productTitle duplicate guard (server.ts /api/reviews/create).
  const eligibleReviewChats = currentUser
    ? chats.filter(c =>
        c.buyerId === currentUser.id &&
        c.sellerId === seller.id &&
        c.tradeStatus === 'completed' &&
        !reviews.some(r => r.buyerId === currentUser.id && (r.productTitle || null) === (c.productTitle || null))
      )
    : [];
  ...
  const handleSubmitReview = async (e: React.FormEvent) => {
    ...
    const selectedChat = eligibleReviewChats.find(c => c.id === selectedReviewChatId);
    if (!selectedChat) {
      setReviewErrorMessage('Please choose which completed trade this review is for.');
      return;
    }
    ...
    setReviewErrorMessage(null);
    try {
      await addReview(seller.id, reviewRating, reviewComment.trim(), selectedChat.productTitle || undefined, selectedChat.id);
      setReviewComment('');
      setReviewRating(5);
      ...
      setReviewErrorMessage(err?.message || 'Could not submit your review. Please try again.');
    } finally {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
      ...
      <button
        ...
        className="flex items-center gap-2 text-sm text-slate-600 hover:text-slate-950 font-semibold mb-6 transition text-left cursor-pointer px-2.5 py-1.5 -ml-2.5 rounded-lg hover:bg-slate-100/80 active:bg-slate-200/70"
        ...
        <div className="flex items-center gap-5">
          ...
          <div className="space-y-1.5">
            <h1 id="seller-profile-title" className="text-xl sm:text-2xl font-bold font-sans tracking-tight text-white flex items-center gap-2 flex-wrap">
              ...
              {isPrioSeller && (
                <span className="inline-flex items-center gap-1 text-[10px] sm:text-xs bg-amber-500 text-slate-950 font-black px-2.5 py-0.5 rounded-full uppercase tracking-wider shadow-sm animate-pulse" title="Active Seller - Boosted Ad Listing">
                  <Flame className="w-3 h-3 text-slate-950 fill-slate-950 animate-bounce" />
                  Active Seller
                </span>
              )}
              {isOwner && (
            ...
            <div className="flex flex-wrap items-center gap-x-4 gap-y-1.5 text-xs text-slate-350 font-sans">
              <span className="flex items-center gap-1">
                ...
                {formatTedbuyTenure(seller.joinDate)}
              </span>
              <span className="flex items-center gap-1">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 text-left pt-2">
        ...
        <div className="lg:col-span-4 space-y-6">
          <div className="flex items-center justify-between">
            ...
            {/* Write Review Button — only offered when a real completed
                trade exists to review; see eligibleReviewChats above. */}
            ...
            {!isOwner && currentUser && eligibleReviewChats.length === 0 && (
              <span className="text-[11px] text-slate-400 italic font-medium">Complete a trade to review</span>
            )}
          </div>
          ...
          <div className="bg-slate-50 border border-slate-200 rounded-3xl p-5 space-y-4 shadow-3xs">
            ...
            <p className="text-[9px] text-slate-400 leading-snug pt-1">
              * Note: The rating matches are aggregated from all completed and reviews-certified sales recorded in our real-time backend database.
            </p>
          </div>
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 font-sans text-slate-800">
            ...
            >
              ...
              <form onSubmit={handleSubmitReview} className="space-y-4">
                ...
                {/* Which completed trade this review is for — REQUIRED,
                    restricted to trades the reviewer actually completed
                    with this seller (eligibleReviewChats), not any listing
                    in their catalog. Replaces the old "Purchased Item
                    (Optional)" free pick / "General Store Review", which
                    let a review claim any product at all with nothing to
                    back it up. */}
                {eligibleReviewChats.length > 1 && (
                  <div>
                    <label className="block text-xs font-bold text-slate-700 mb-1">Which trade is this review for?</label>
                    <select
                      value={selectedReviewChatId || ''}
                      onChange={(e) => setSelectedReviewChatId(e.target.value)}
                    ...
                    >
                      {eligibleReviewChats.map(c => (
                        <option key={c.id} value={c.id}>{c.productTitle || 'Completed Trade'}</option>
                      ))}
                    </select>
                  </div>
                )}
