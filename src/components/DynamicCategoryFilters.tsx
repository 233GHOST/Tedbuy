...
export const DynamicCategoryFilters: React.FC<DynamicCategoryFiltersProps> = ({
  ...
  const activeFiltersCount = useMemo(() => {
    ...
    count += Object.keys(extraFilters).filter(k => Boolean(extraFilters[k])).length;
    return count;
  }, [extraFilters, minPrice, maxPrice]);

  // Controls dropdown expansion state. On mobile screens (< 768px), it defaults to collapsed (dropdown) so listings show right away.
  ...
  // Whenever the user switches categories on mobile, collapse the dropdown so the listings under that category are immediately visible
  useEffect(() => {
  ...
  return (
    <div className="bg-white border border-slate-200/90 rounded-2xl sm:rounded-3xl shadow-xs overflow-hidden transition-all text-left font-sans animate-fade-in">
      {/* Dropdown Header Trigger - Clear and prominent on mobile */}
      <div
        id="btn-filter-category-dropdown"
        onClick={() => setIsExpanded(prev => !prev)}
        className="w-full flex items-center justify-between p-3 sm:p-4 bg-white hover:bg-slate-50/90 active:bg-slate-100 transition-colors cursor-pointer select-none border-b border-transparent"
        role="button"
        tabIndex={0}
      ...
      >
        <div className="flex items-center gap-2.5 sm:gap-3 min-w-0">
          <div className="w-8 h-8 sm:w-9 sm:h-9 rounded-xl bg-emerald-50 border border-emerald-200/80 flex items-center justify-center shrink-0 text-emerald-600 shadow-3xs">
            <SlidersHorizontal className="w-4 h-4 sm:w-4.5 sm:h-4.5 stroke-[2.5]" />
          </div>
          <div className="flex items-center gap-2 flex-wrap min-w-0">
            <h4 className="text-xs sm:text-sm font-black text-slate-900 uppercase tracking-tight flex items-center gap-1.5 truncate">
              <span>FILTER {selectedCategory}</span>
            </h4>
            {activeFiltersCount > 0 && (
              <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-black bg-emerald-500 text-white shadow-3xs">
        ...
        <div className="flex items-center gap-2 shrink-0">
          {isAnyFilterActive && (
            <button
              ...
              className="text-[10px] sm:text-xs bg-slate-100 hover:bg-red-50 text-slate-600 hover:text-red-600 border border-slate-200 hover:border-red-200 font-bold px-2.5 py-1 rounded-xl transition-all flex items-center gap-1 cursor-pointer"
              title="Reset all active category filters"
            >
          ...
          {/* High-visibility Dropdown Button Pill */}
          <div className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-black shadow-3xs tracking-tight transition-all duration-200 ${
            isExpanded 
              ? 'bg-slate-900 text-white' 
              : 'bg-emerald-50 text-emerald-800 border border-emerald-200/90 hover:bg-emerald-100'
          }`}>
            <span className="text-[10px] sm:text-xs uppercase font-extrabold">
              {isExpanded ? 'Hide' : 'Filter'}
            </span>
            <ChevronDown
              className={`w-3.5 h-3.5 sm:w-4 sm:h-4 transition-transform duration-300 ${
                isExpanded ? 'rotate-180 text-emerald-400' : 'text-emerald-700'
              }`}
            />
          </div>
        </div>
      ...
      {isExpanded && (
        <div className="p-4 sm:p-5 pt-3 border-t border-slate-100 space-y-4 animate-fade-in">
          {/* Price Range Section (Quick Price Range + Custom Price Range Inputs) */}
          <div className="space-y-1.5">
            <div className="flex items-center justify-between">
              ...
              {(minPrice || maxPrice) && (
                <button
                  ...
                  className="text-[9px] text-red-600 hover:text-red-700 hover:underline font-bold flex items-center gap-0.5 cursor-pointer"
                >
            ...
            <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2">
                ...
                {priceRanges.map((range) => {
                  ...
                  return (
                    <button
                      ...
                      className={`px-2 py-1.5 rounded-xl border text-[10px] font-bold text-center transition-all cursor-pointer select-none truncate ${
                        isSelected
                          ? 'bg-emerald-500 border-emerald-500 text-white shadow-xs font-extrabold'
              ...
              {/* Custom Min - Max Price Range Inputs */}
              <div className="flex items-center gap-1.5 bg-slate-50 border border-slate-200 rounded-xl px-2.5 py-1 shrink-0 sm:w-52">
                <span className="text-[10px] font-extrabold text-slate-400 uppercase tracking-tight shrink-0">GH₵</span>
                <input
                  type="number"
                  placeholder="Min"
                  value={minPrice}
                  onChange={(e) => setMinPrice && setMinPrice(e.target.value)}
                  className="w-full min-w-0 bg-white border border-slate-200 text-slate-800 text-xs font-bold rounded-lg px-2 py-1 focus:outline-none focus:ring-1 focus:ring-emerald-500 shadow-3xs font-mono placeholder:font-sans placeholder:font-normal"
                />
                <span className="text-slate-350 text-xs font-extrabold shrink-0">-</span>
                <input
                  type="number"
                  placeholder="Max"
                  value={maxPrice}
          ...
          {brandGridOptions && (
            <div className="space-y-2 pt-1 border-t border-slate-100">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-7 gap-2">
                {brandGridOptions.items.map((brand) => {
                  ...
                  return (
                    <button
                      ...
                      className={`p-2.5 rounded-2xl border flex flex-col items-center justify-center gap-1.5 transition-all duration-200 cursor-pointer select-none ${
                        isSelected
                          ? 'border-emerald-500 bg-emerald-50 text-emerald-800 ring-1 ring-emerald-500/30 font-bold shadow-xs'
                ...
                >
                  <div className="h-9 flex items-center justify-center text-xs font-black text-slate-400">
                    •••
                  </div>
          ...
          {/* Mobile Done & View Listings Button */}
          <div className="sm:hidden pt-2 border-t border-slate-100">
            <button
              type="button"
              onClick={() => setIsExpanded(false)}
              className="w-full py-2.5 bg-slate-900 hover:bg-slate-800 active:scale-98 text-white text-xs font-black rounded-xl shadow-xs flex items-center justify-center gap-2 cursor-pointer transition"
            >
              <Check className="w-3.5 h-3.5 text-emerald-400 stroke-[3]" />
              <span>Done & Show Listings</span>
            </button>
          </div>
        </div>
      )}
