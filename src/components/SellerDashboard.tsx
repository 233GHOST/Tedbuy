...
import { Edit2, Trash2, PlusCircle, Eye, ShoppingBag, MapPin, Tag, Plus, Bookmark, AlertTriangle, Play, Sparkles, Clock, Flame, Users, Loader2 } from 'lucide-react';
                ...
                >
                  ...
                  <div className="col-span-1 md:col-span-6 flex gap-4 text-left">
                    {prod.images && prod.images.length > 0 ? (
                      ...
                      />
                    ) : (prod.videos && prod.videos.length > 0 && isVideoAsset(prod.videos[0])) ? (
                    ...
                    <div className="flex flex-col justify-center min-w-0">
                      {!(prod.category && (prod.category.toLowerCase() === 'services' || prod.category.toLowerCase().includes('service'))) ? (
                        <span className="text-base font-bold text-slate-950 font-sans flex items-center gap-2 flex-wrap">
                          GHS {prod.price ? prod.price.toLocaleString() : '0'}
                          {prod.isSold && (
                          ...
                          {isBoostActive(prod) && (
                            <span className="bg-amber-500 text-slate-950 border border-amber-600 text-[9px] px-2 py-0.5 rounded-md font-black uppercase tracking-wider shadow-3xs flex items-center gap-1 shrink-0 animate-pulse">
                        ...
                        (prod.isSold || isBoostActive(prod)) && (
                      ...
                      {isBoostActive(prod) && (
                        <span className="text-[10px] text-amber-700 font-bold mt-1.5 flex items-center gap-1 bg-amber-50 border border-amber-200/50 rounded-lg px-2 py-0.5 w-fit">
                          ...
                          <span>Expires: {getBoostEndDate(prod)?.toLocaleString('en-US', { month: 'short', day: 'numeric', year: 'numeric', hour: '2-digit', minute: '2-digit' }) || ''}</span>
                        </span>
                      )}
                    </div>
                  </div>
