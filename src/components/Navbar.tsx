    ...
    <>
      {!showAuthModal && (
        ...
        <div className="flex items-center justify-between h-16 gap-4">
          ...
          {/* Search bar inside header (Always available on desktop so users can search from anywhere) */}
          <div className="flex-1 max-w-xl mx-2 sm:mx-4 relative hidden md:block">
            ...
            <input
              ...
              className="block w-full pl-10 pr-9 py-2 border border-slate-700 focus:border-slate-500 rounded-xl bg-slate-800 text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-slate-600/50 text-sm transition"
            />
            {searchQuery && (
              <button
                type="button"
            ...
            {isDesktopFocused && searchQuery.trim().length > 0 && (
              <SearchSuggestions
                ...
                onSelectQuery={(q) => {
                  ...
                  if (currentView !== 'browse') setCurrentView('browse');
                  if (homeViewMode === 'video-feed') setHomeViewMode('grid');
                  window.scrollTo({ top: 0, behavior: 'smooth' });
                }}
                ...
                onRemoveRecentSearch={(term) => removeRecentQuery(term)}
                onClearRecentSearches={() => clearRecentSearches()}
                onClose={() => setIsDesktopFocused(false)}
                theme="light"
              />
            )}
          </div>
                  ...
                  >
                    ...
                    <div className="flex flex-col text-left hidden lg:block leading-none">
                      ...
                      <span className="text-[9px] text-slate-400 font-medium mt-0.5">
                        {formatTedbuyTenure(currentUser.joinDate)}
                      </span>
                    </div>
                  </div>
                  <button
                ...
                isVerifyingOtp ? (
                  <div className="space-y-5 animate-fade-in">
                    <div className="text-center">
                      <div className="mx-auto flex h-12 w-12 items-center justify-center rounded-full bg-emerald-100 mb-3 text-emerald-600 ring-8 ring-emerald-50/50">
                    ...
                    <div>
                      ...
                      <input
                        ...
                        placeholder="000000"
                        className="w-full text-center tracking-[0.5em] font-mono text-2xl px-3.5 py-2.5 rounded-xl bg-white border border-slate-300 text-slate-900 focus:outline-none focus:ring-2 focus:ring-emerald-500 placeholder-slate-300 transition"
                      />
                    </div>
                    ...
                    <div className="flex flex-col gap-2.5 pt-2">
                      <button
                        ...
                        className="w-full flex items-center justify-center gap-2 py-3 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-sm font-bold text-white transition duration-150 shadow-sm cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed"
                      >
                        {isAuthSubmitting ? (
                          <>
