...
export const Navbar: React.FC = () => {
  ...
  return (
    <header className="sticky top-0 z-40 bg-slate-900 border-b border-slate-800 text-white shadow-md">
      ...
      <div className="bg-emerald-600 text-white font-mono text-[11px] font-bold px-4 py-1.5 flex flex-wrap justify-between items-center gap-2">
        <div className="flex items-center gap-1.5">
          <span className="relative flex h-2 w-2">
            <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-slate-950 opacity-75"></span>
            <span className="relative inline-flex rounded-full h-2 w-2 bg-slate-950"></span>
          </span>
          <span>DEV MODE: SIMULATE CLASSIFIEDS USERS</span>
        </div>
        <div className="flex items-center gap-2">
          <span className="opacity-90">Switch Active Test Profile:</span>
            ...
            {users.map(u => (
              <button
                ...
                className={`px-2 py-0.5 rounded text-[10px] font-bold transition-all ${
                  currentUser?.id === u.id
                    ? 'bg-slate-950 text-white ring-1 ring-white'
                    : 'bg-emerald-750 text-emerald-50 hover:bg-emerald-800'
                }`}
              >
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16 gap-4">
          ...
          >
            <div className="w-10 h-10 rounded-xl bg-emerald-650 flex items-center justify-center shadow-sm group-hover:scale-105 transition-transform duration-200">
              <ShoppingBag className="w-5.5 h-5.5 text-white stroke-[2.5]" />
            </div>
            <div className="flex flex-col">
              <span className="text-xl font-sans font-black tracking-tight leading-none bg-gradient-to-r from-emerald-400 to-emerald-550 bg-clip-text text-transparent">
                tedbuy
              </span>
              <span className="text-[10px] text-slate-400 font-mono tracking-wider">CLASSIFIEDS</span>
            </div>
          </div>
          ...
          <div className="flex-1 max-w-lg relative hidden md:block">
            ...
            <input
              ...
              className="block w-full pl-10 pr-4 py-2 border border-slate-700 rounded-xl bg-slate-800 text-slate-100 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent text-sm transition"
            />
          </div>
            ...
            {currentUser ? (
              <div className="flex items-center gap-2 pl-2 border-l border-slate-800">
                <img
                  ...
                  className="w-8 h-8 rounded-full border border-slate-700 object-cover hidden sm:block"
                />
                <div className="flex flex-col text-left hidden lg:block leading-none">
                  <span className="text-xs font-semibold text-slate-200 block truncate max-w-[90px]">
                ...
                <button
                  ...
                  onClick={logoutUser}
                  title="Sign Out"
                  className="p-2 text-slate-400 hover:text-red-400 rounded-xl hover:bg-slate-800/60 transition"
                >
              ...
              <button
                ...
                className="px-4 py-2 bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-sm rounded-xl transition duration-200 flex items-center gap-1.5 shadow-md shadow-emerald-600/10"
              >
        ...
        <div className="pb-3 md:hidden">
          <div className="relative">
            ...
            <input
              ...
              className="block w-full pl-9 pr-3 py-1.5 border border-slate-700 rounded-xl bg-slate-800 text-slate-100 placeholder-slate-450 focus:outline-none focus:ring-1 focus:ring-emerald-500 text-xs transition"
            />
          </div>
        </div>
      </div>
      ...
      {showAuthModal && (
        <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-xs flex items-center justify-center p-4 z-50">
          <div className="bg-slate-900 border border-slate-850 rounded-2xl p-6 max-w-md w-full shadow-2xl relative text-left">
            <button
            ...
            {authError && (
              <div id="auth-error-msg" className="bg-red-950/50 text-red-400 p-3 rounded-lg text-xs mb-4 border border-red-900">
                {authError}
              </div>
            )}
            ...
            <form onSubmit={handleAuthSubmit} className="space-y-4">
              {authMode === 'register' && (
                <>
                  <div>
                    <label className="block text-xs font-semibold text-slate-350 mb-1.5">Username / Store Name *</label>
                    <input
                      type="text"
                      ...
                      className="w-full px-3.5 py-2 rounded-xl bg-slate-800 border border-slate-700 text-sm text-slate-100 focus:outline-none focus:ring-2 focus:ring-emerald-500 placeholder-slate-500"
                    />
                  </div>
                  <div>
                    <label className="block text-xs font-semibold text-slate-350 mb-1.2">Email Address <span className="text-slate-500 text-[10px] font-normal">(Optional if phone is provided)</span></label>
                    <input
                      type="email"
                      id="auth-email-input"
                  ...
                  <div>
                    <label className="block text-xs font-semibold text-slate-350 mb-1.2">Phone Number <span className="text-slate-500 text-[10px] font-normal">(Optional if email is provided)</span></label>
                    <input
                      type="text"
                      id="auth-phone-input"
              ...
              {authMode === 'login' && (
                <div>
                  <label className="block text-xs font-semibold text-slate-350 mb-1.5">Email Address or Phone Number</label>
                  <input
                    type="text"
              ...
              <button
                type="submit"
                id="auth-submit-btn"
                className="w-full py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white font-bold rounded-xl transition duration-200 text-sm"
              >
            ...
            <div className="mt-4 bg-slate-800/40 p-3 rounded-xl border border-slate-800 text-[10px] text-slate-400">
              <span className="font-bold block text-slate-300 mb-1">💡 Quick Simulator Presets (Click to Auto-Fill):</span>
              <div className="grid grid-cols-2 gap-1.5 mt-1">
                <button
                  ...
                  className="bg-slate-800 hover:bg-slate-750 p-1.5 rounded text-slate-300 border border-slate-750 text-left font-mono truncate"
