...
export const AppProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  //
  // Security fix (RLS-migration Phase 0): this used to be a direct, fully
  // unauthenticated `getDocs(collection(null, 'users'))` -- a `select('*')`
  // against the entire users table, no auth, no column restriction. That
  // returned every user's email/phoneNumber/whatsAppNumber/isAdmin/
  // isSuspended/securityHold to any anonymous browser, for every user, in
  // one request. GET /api/users/list already existed (built for mobile's
  // equivalent feature) and was already PII-safe -- it deliberately omits
  // contact info specifically because it returns everyone in one response;
  // web had simply never been switched to it. This `users` state now only
  // ever carries the safe fields that endpoint returns; the two features
  // that legitimately need a specific user's contact info now fetch that
  // one user's full profile directly instead (see SellerProfilePage.tsx's
  // seller-contact lookup, and sendWelcomeEmailToAll's own admin-gated
  // bulk fetch below) -- see
...
${comment ? `• Comments: "${comment}"` : ''}`;
  ...
  const sendWelcomeEmailToAll = async (
    ...
    // Security fix (RLS-migration Phase 0): this used to filter the shared
    // `users` state, which came from an unauthenticated bulk table read and
    // included email for that reason. Now that `users` only ever carries
    // the safe fields GET /api/users/list returns (no email), this
    // admin-only feature fetches its own admin-gated bulk list with
    // contact info instead of relying on that shared state.
  ...
  const adminDeleteUserProfile = async (userId: string, forceDeleteActive: boolean = false) => {
    ...
    // work: it relied on `targetUser.email`, which the shared `users` state
    // no longer carries now that it's sourced from the PII-safe
    // GET /api/users/list instead of an unauthenticated bulk table read
    // (see the fix note on the `users`-fetching effect above). This was
    // always documented as UX-only -- /api/admin/users/delete independently
    // re-fetches the real row and re-checks the same guard server-side
    // before doing anything, so removing the client-side copy has no
    // security effect, just a slightly later error message.
  ...
  const adminToggleUserSuspension = async (userId: string, suspend: boolean) => {
    ...
    // Same removal, same reason as adminDeleteUserProfile above: the
    // client-side super-admin guard needed `targetUser.email`, which the
    // shared `users` state no longer carries after the RLS-migration
    // Phase 0 fix. /api/admin/users/suspend already independently
    // re-verifies the same guard against the real row.
    const authHeaders = await getAuthHeader();
  ...
  const adminToggleSecurityHold = async (userId: string, hold: boolean, reason?: string) => {
    ...
    // Same removal, same reason as adminDeleteUserProfile above -- and
    // this one used to be the ONLY check preventing a security hold on the
    // super-admin account at all (unlike delete/suspend, the server
    // endpoint had no independent re-check of its own). Fixed on the
    // server side too as part of this same pass (see
