...
export interface User {
  ...
  securityHoldSetAt?: string;
  securityHoldSetBy?: string;
  fcmTokens?: string[];
  visitCount?: number;
...
export interface Product {
  ...
  brand?: string;
  condition?: string;
  images: string[]; // 1 to 10 images (Cloudinary URLs)
  imageUrls?: string[];
  thumbnailUrls?: string[];
  videos?: string[]; // Optional: 1 to 2 videos (Cloudinary URLs)
  videoUrls?: string[];
  videoPoster?: string;
  displayImage?: string; // Canonical single source of truth primary display image
  primaryImage?: string;
  thumbnailUrl?: string;
...
export function normalizeCategory(cat: any): Category {
  ...
  if (clean.includes('furniture') || clean.includes('sofa') || clean.includes('chair') || clean.includes('table') || clean.includes('bed') || clean.includes('decor')) return 'Furniture & Home';
  ...
  if (clean.includes('job') || clean.includes('employment') || clean.includes('hiring') || clean.includes('vacancy') || clean.includes('resume')) return 'Jobs & Employment';
  if (clean === 'service' || clean === 'services' || clean.includes('service') || clean.includes('freelance') || clean.includes('repair')) return 'Services';
  if (clean.includes('agriculture') || clean.includes('farm') || clean.includes('crop') || clean.includes('poultry') || clean.includes('seed')) return 'Agriculture & Food';
  if (clean.includes('pet') || clean.includes('animal') || clean.includes('dog') || clean.includes('cat') || clean.includes('puppy')) return 'Pets & Animals';
  if (clean.includes('sport') || clean.includes('fitness') || clean.includes('gym') || clean.includes('bicycle')) return 'Sports & Fitness';
  if (clean.includes('kid') || clean.includes('baby') || clean.includes('child') || clean.includes('stroller') || clean.includes('toy')) return 'Kids & Baby';
  if (clean.includes('commercial') || clean.includes('tool') || clean.includes('machinery') || clean.includes('industrial') || clean.includes('generator')) return 'Commercial & Tools';
  if (clean.includes('book') || clean.includes('novel') || clean.includes('hobby') || clean.includes('instrument') || clean.includes('music')) return 'Books & Hobbies';
