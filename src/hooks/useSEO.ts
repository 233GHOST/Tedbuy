...
export function useSEO({
  ...
  useEffect(() => {
    ...
    if (currentView === 'product-detail' && selectedProductId) {
      ...
      canonical = `${origin}/?q=${encodeURIComponent(searchQuery.trim())}`;
    }
    ...
    // Manage robots indexing to prevent Soft 404 on deleted/non-existent items
    const isMissingProduct = currentView === 'product-detail' && selectedProductId && !products.find(p => p.id === selectedProductId);
    const isMissingSeller = currentView === 'seller-profile' && selectedSellerId && !seller;
    if (isMissingProduct || isMissingSeller) {
