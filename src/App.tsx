...
const MarketplaceContent: React.FC = () => {
  ...
  const [resetPasswordEmailAddress, setResetPasswordEmailAddress] = useState<string | null>(null);
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (oobCode) {
        console.log('[App] Reset password oobCode detected in URL:', oobCode);
        setIsResetPasswordModalOpen(true);
        setIsVerifyingResetCode(true);
        setResetPasswordOobCode(oobCode);
        setResetPasswordError(null);

        fetch('/api/auth/verify-password-reset-code', {
          method: 'POST',
          ...
          .then(data => {
            if (data.success && data.email) {
              console.log('[App] Password reset code verified for email:', data.email);
              setResetPasswordEmailAddress(data.email);
            } else {
              console.error('[App] Password reset code verification failed:', data.error);
              setResetPasswordError('This password reset link is invalid or has expired. Please request a new password reset link.');
            }
          })
          .catch((err) => {
            console.error('[App] Failed to verify password reset code:', err);
            setResetPasswordError('This password reset link is invalid or has expired. Please request a new password reset link.');
          })
          .finally(() => {
      ...
      console.warn('[App] URL parameter parsing warning:', e);
    }
  }, []);
  ...
  React.useEffect(() => {
    // If in All Categories view without search, do not observe sentinel or fetch more products on scroll
    ...
    const observer = new IntersectionObserver((entries) => {
      ...
      rootMargin: '350px', // start loading before the user completely reaches the bottom
      threshold: 0.1
    });
    ...
    const handleScrollFallback = () => {
      if (isAllCategoriesView || !scrollSentinelRef.current) return;
  ...
  }, [scrollSentinelRef.current, sortedProducts.length, hasMoreProducts, isLoadingMoreProducts, loadMoreProducts, isAllCategoriesView]);
  ...
  return (
    <div className={`flex flex-col font-sans relative ${
        ...
        {currentView === 'browse' && (
          ...
          >
            ...
            {homeViewMode !== 'video-feed' && (
              ...
              <div className="relative mb-8 bg-slate-100 border border-slate-200 text-slate-900 rounded-3xl p-5 sm:p-6 shadow-xs">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-end relative z-30">
                  ...
                  <div className="lg:col-span-6 text-left w-full">
                    ...
                    <div className="relative">
                      ...
                      {/* Enhanced Autocomplete & Live Search Suggestion Dropdown Panel */}
                      {isSearchFocused && searchQuery.trim().length > 0 && (
                        <SearchSuggestions
                          ...
                          onRemoveRecentSearch={(term, e) => removeRecentSearch(term, e)}
                  ...
                  {/* Right Column: Ghana Location Filters (Positioned where marked blue on desktop) */}
              ...
              {showAllCategories ? (
                ...
                <div className="relative group/scroll flex items-center">
                  ...
                  <div 
                    ...
                    className="flex gap-2 pb-2 overflow-x-auto w-full px-7 scrollbar-none snap-x snap-proximity touch-auto overscroll-x-contain"
                    style={{ scrollbarWidth: 'none', msOverflowStyle: 'none', WebkitOverflowScrolling: 'touch' }}
                  >
                    <button
                      id="category-tag-all"
                      onClick={() => setSelectedCategory(null)}
                      className={`px-4 py-2.5 rounded-xl text-xs font-bold flex items-center gap-1.5 transition whitespace-nowrap border cursor-pointer snap-start ${
                        selectedCategory === null
            ...
            {/* Full-width main feed section */}
            <div className="w-full space-y-6">
              {/* Category-specific Extra Filters & Price Range Bar when applicable */}
              {(selectedCategory || debouncedSearchQuery.trim() !== '') && (
                <div className={`grid grid-cols-1 ${debouncedSearchQuery.trim() !== '' && selectedCategory ? 'md:grid-cols-2' : 'w-full'} gap-4`}>
                  <div className={debouncedSearchQuery.trim() !== '' && selectedCategory ? '' : 'col-span-full w-full'}>
                  ...
                  {/* Price Budget Filter - Shown when searching for a product */}
              ...
              {recentlyViewedProducts.length > 0 && (
                <div
                  ...
                  className="bg-white border border-slate-200 rounded-3xl p-4 shadow-xs space-y-3 text-left font-sans animate-fade-in mb-6"
                >
                  <div className="flex items-center justify-between pb-1.5 border-b border-slate-100">
                    ...
                    <button
                      ...
                      className="text-[10px] text-slate-400 hover:text-red-500 font-extrabold hover:bg-red-50 px-2 py-0.5 rounded-md transition duration-250 cursor-pointer"
                    ...
                    {recentlyViewedProducts.map(product => {
                      ...
                      return (
                        <div
                          ...
                          className="flex items-center gap-2.5 p-1.5 rounded-2xl border border-slate-100 hover:border-slate-300 hover:bg-slate-50 transition cursor-pointer group select-none"
                        >
                          <div className="relative w-10 h-10 rounded-xl overflow-hidden bg-slate-100 border border-slate-150 shrink-0">
                            {product.videos && product.videos.length > 0 ? (
                              ...
                              <img
                                src={resolveProductImage(product, 100)}
                                alt={product.title}
                                loading="lazy"
                                decoding="async"
                                className="w-full h-full object-cover transition duration-300 group-hover:scale-110"
              ...
              {!debouncedSearchQuery.trim() && !searchQuery.trim() && (
                <TrendingListings selectedCategory={selectedCategory} />
              )}
              ...
              <section className="space-y-6">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-slate-200 pb-4 text-left">
                  ...
                  <div className="flex flex-wrap items-center gap-3.5 self-start sm:self-center">
                    ...
                    {/* Price Range Filter */}
                    <div className="flex items-center gap-1.5">
                      <label htmlFor="min-price-input" className="text-xs font-bold text-slate-500 whitespace-nowrap">Price Range (GH₵):</label>
                      <div className="flex items-center gap-1">
                        <input
                          ...
                          value={minPrice}
                          onChange={(e) => setMinPrice(e.target.value)}
                          className="w-16 bg-white border border-slate-200 text-slate-700 text-xs font-semibold rounded-xl px-2.5 py-1.5 focus:outline-none focus:ring-2 focus:ring-slate-350 shadow-3xs transition hover:border-slate-300 font-mono"
                        />
                ...
                {isProductsLoading && sortedProducts.length === 0 ? (
                  ...
                  <div className="space-y-6">
                    ...
                    {/* 4 Product Cards Per Row on Desktop */}
                    <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4 sm:gap-6 animate-fade-in">
                      {(isAllCategoriesView ? sortedProducts.slice(0, 24) : sortedProducts.slice(0, displayLimit)).map((product, index) => (
              ...
              <div className="col-span-1 space-y-3.5">
                ...
                <ul className="space-y-2.5 text-xs">
                  {[
                    ...
                    { label: 'Furniture & Living', cat: 'Furniture & Home' },
                    { label: 'Jobs & Vacancies', cat: 'Jobs & Employment' },
                    { label: 'Agriculture & Produce', cat: 'Agriculture & Food' },
                    { label: 'Services & Artisans', cat: 'Services' }
                  ].map((item) => (
                    <li key={item.cat}>
                      <button
                        onClick={() => {
      ...
      <ResetPasswordModal
        ...
        email={resetPasswordEmailAddress}
        oobCode={resetPasswordOobCode}
        error={resetPasswordError}
        isVerifying={isVerifyingResetCode}
        onSuccess={() => {
          setIsResetPasswordModalOpen(false);
          if (typeof window !== 'undefined') {
            window.history.replaceState({}, '', window.location.pathname);
